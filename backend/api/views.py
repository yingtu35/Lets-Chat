...
@api_blueprints.route('/signin', methods=["POST"])
def signin():
...
@api_blueprints.route('/logout', methods=["POST"])
def logout():
...
@api_blueprints.route('/signup', methods=["POST"])
def signup():
    data = request.get_json()
    username = data.get("username")
    password = data.get("password")
...
def room_leave(username):
    ...
    if not login_username or not rid:
        # clear cookies
        session.pop('username', None)
        session.pop('room', None)
